...
The repository evidence recommends retaining the two-pane information architecture and refining its visual hierarchy. After exploring replacement structures on 2026-08-29, the user explicitly deferred structural optimization and chose visual alignment only. The implementation must therefore preserve the existing topology, General destination, feature IDs, host-driven selection, localization, fixed-window constraints, and Settings behavior.
