...
try {
  # Preflight is deliberately repeated after elevation; the elevated process never trusts this pass.
  $package = Test-ClacklyPackage -PackageRoot $packageRoot
  ...
  $result = Invoke-ClacklyInstallTransaction -Package $package -PluginRoot (Get-ClacklyProductionPluginRoot)
  ...
  Write-Host ("Clackly Beta installer stopped before any install transaction: " + $_.Exception.Message)
