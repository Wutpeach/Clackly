...
COMMAND_HANDLERS: Dict[str, Callable[[Dict[str, Any]], Dict[str, Any]]] = {
    "timeline.addMarker": lambda _payload: add_marker(),
    "media.clipboard-image.project": lambda _payload: get_current_project_name(),
    "media.clipboard-image.import": lambda payload: import_media_to_bin(
        payload.get("diskPath"), payload.get("binName")
    ),
}


def execute_command(
    command_id: str, payload: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
