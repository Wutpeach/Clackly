...
Command manifests require `id`, `name`, `description`, `category`, `icon`, `keywords`, and `capability`. The Command Registry validates and defensively projects that fixed shape through list, search, and lookup. Production Launcher and Search presentation uses only those registered records. The single hostless Vite root preview is the narrow exception: without `window.resolveCommandCenter`, it uses an isolated renderer-local adapter with representative Commands, lifecycle status, and normalized bindings so the Palette and Interaction Panel can be inspected in a browser. That adapter is non-executable and is never Registry, preload, IPC, native-window, or Resolve authority.
...
`InteractionManager.listBindings()` exposes normalized defensive binding records through the same semantic preload API in both Electron hosts. One renderer projection selects bindings for the hovered/focused target Command, resolves each `action.command` against loaded Command Metadata, and formats the generic trigger label (`Click`, `Shift + Right Click`, and so on) together with the registered action label and description. Palette and Settings share this projection, so remapping a binding updates help without changing Command Metadata or renderer branches. Missing action metadata omits only that help row; empty bindings fall back to the target Command description.

## Command Interaction Hint

The Palette Footer shows an Info control for every selected Command. Clicking Info or pressing `Tab` while Command selection has focus opens a read-only Interaction Panel. On Windows that surface is the detached D7 native Panel; on non-Windows it uses the compatible attached fallback. The panel consumes the current Palette selection directly: more than one resolved interaction renders only compact input-to-action mappings, while a default/single or unresolved interaction renders only the registered Command description. It has no copied Command context, search, row selection, acknowledgement, or empty state. `Tab`, `Esc`, selection change, execution, and Palette close end the panel lifecycle.

`npm run palette:evidence` is the developer-only Playwright renderer utility. It runs headlessly by default against packaged assets and writes temporary evidence; use `node scripts/palette-evidence.mjs --renderer built --scenario interaction-panel,interaction-lifecycle --output <directory>` to select a built renderer, scenarios, and output. `--headed` is an explicit opt-in. The tool injects the preload-shaped host API plus registered Commands and normalized interaction bindings only inside the browser process. Its evidence is DOM-only: it does not validate DWM composition, HWND separation, native hit testing, focus, z-order, packaged runtime, Resolve behavior, or command execution.
...
The Settings button opens a fixed, frameless `760x560` window with a draggable Clackly title bar and accessible custom close button. On Windows, Launcher and Search remain on the opaque, full-bleed `240x320` D6 main Palette while Interaction Info opens in the separate persistent D7 native Panel. D7 is `260px` wide, content-fit within `60–180px`, permanently nonfocusable, and separated from D6 by a real `16px` desktop gap. The host clamps both native windows as one composition and restores the exact pre-Panel main bounds on close; visibility uses immediate opacity and mouse gating. Both D6 and D7 keep `roundedCorners:true` and `thickFrame:true` so DWM owns their outer corners and shadow, and neither uses `setShape`. Only the compatible non-Windows transparent attached fallback retains the shaped main-plus-Panel union. Settings keeps its independent window contract and scrolls overflow inside the fixed workspace. `feature-ui/FeatureCatalog.js` projects full defensive metadata records from the existing Capability Registry, so registering a capability automatically adds it to the category-grouped Settings sidebar.
...
Resolve 20.3.2.9 with its bundled Electron 36.3.2 is the qualified host baseline. The local Electron dependency is pinned exactly to 36.3.2 so development and automated window-contract tests cannot silently adopt newer APIs. Native window behavior must be validated through the Resolve Workflow Integration plugin: standalone Electron tests and renderer screenshots cannot prove Resolve-hosted DWM corners and shadow, the real `16px` inter-window gap, focus retention, z-order, or native hit testing across the D6/D7 lifecycle.
